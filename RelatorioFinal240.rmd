...
Por meio do tamanho da lista gerada ao importar os arquivos, é possível obter a quantidade de professores do __Programa de Pós-graduação em Biologia Animal__ no período de 2010 a 2017.
...
*Retorna campos dentro do arquivo `advise`*
```{r}
names(orient)
```
...
```{r}
length(orient$ORIENTACAO_CONCLUIDA_MESTRADO$`2017`$natureza)
```


*Quantidade de orientações de Doutorado concluídas em 2017*
```{r}
length(orient$ORIENTACAO_CONCLUIDA_DOUTORADO$`2017`$natureza)
```


*Cursos presentes nos Doutorados concluídos em 2017*
```{r}
head(sort(table(orient$ORIENTACAO_CONCLUIDA_DOUTORADO$`2017`$curso), decreasing = TRUE), 10)
```


*Cursos presentes nos Mestrados concluídos em 2017*
```{r}
head(sort(table(orient$ORIENTACAO_CONCLUIDA_MESTRADO$`2017`$curso), decreasing = TRUE), 10)
```
...
*Retorna campos com tipos de publicação dentro de `publication`*
```{r}
names(public)
```
...
```{r}
names(public$PERIODICO$`2017`)
```


*Mostra o título dos periódicos publicados em 2017*
```{r}
head(sort(table(public$PERIODICO$`2017`$titulo), decreasing = TRUE), 10)
```
...
de Biologia Animal apresentou um crescimento considerável de 2013 para 2014,
