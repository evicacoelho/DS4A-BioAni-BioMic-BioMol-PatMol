...
for (i in 1:length(perfil.240)) {
  ProfileList <- rbind(ProfileList, perfil.240[[i]]$nome)
}

length(perfil.242)
ProfileLists.242 <- list()
for (i in 1:length(perfil.242)) {
  ProfileList <- rbind(ProfileList, perfil.242[[i]]$nome)
}

length(perfil.243)
ProfileLists.243 <- list()
for (i in 1:length(perfil.243)) {
  ProfileList <- rbind(ProfileList, perfil.243[[i]]$nome)
}

length(perfil.313)
ProfileLists.313 <- list()
for (i in 1:length(perfil.313)) {
  ProfileList <- rbind(ProfileList, perfil.313[[i]]$nome)
}
```
...
df.240 <- as.data.frame(V(g)$name); colnames(df) <- "Idlattes"
...
df.313 <- as.data.frame(V(g.313)$name); colnames(df.313f) <- "Idlattes"
...
V(g.240)$publicacao <- perfil.240.df$PERIODICO
V(g.240)$eventos <- perfil.240.df$EVENTO

df.242 <- df.242 %>% group_by(Idlattes) %>% 
  slice(1L)
V(g.242)$programa <- df$Programa
...
V(g.242)$publicacao <- perfil.242.df$PERIODICO
V(g.242)$eventos <- perfil.242.df$EVENTO
...
V(g.243)$publicacao <- perfil.243.df$PERIODICO
V(g.243)$eventos <- perfil.243.df$EVENTO
...
V(g.313)$publicacao <- perfil.313.df$PERIODICO
V(g.313)$eventos <- perfil.313.df$EVENTO
```
...
```{r}
ggplot(orient.240.df,aes(ano,fill=natureza)) +
...
```{r}
orient.240.df %>% filter(bolsa == "SIM") %>%
...
```{r}
ggplot(perfil.240.df.orientacoes, aes(natureza,fill=orientacao)) +
  geom_bar(stat = 'count') +
...
### Orientações por ano:
```{r}
ggplot(perfil.240.df.orientacoes, aes(ano, fill=orientacao)) +
  geom_bar(stat = 'count') +
...
```{r}
ggplot(orient.240.mestrado.df, aes(ano, fill=curso)) +
  geom_bar(stat = 'count') +
  ggtitle('Orientações por ano') +
  theme(legend.position = 'right') +
  labs(x='Ano',y='Cursos')
